...
The PAR endpoint is served at `Config.Endpoints.PushedAuthorization` (default `/par`) and advertised in discovery. At the authorization endpoint, grantor redeems each `request_uri` once, uses only the pushed parameters, and validates them again against the current client registration; the rest of the flow, including `Interact` and `Approve`, is unchanged, and `AuthorizationRequest.Pushed` tells you the request was pushed. Because `ParseAuthorizationRequest` uses up the `request_uri`, a custom authorization endpoint must approve, deny or save the parsed request in the same HTTP request and continue from the saved request, never by parsing the original URL again. `ParsePushedAuthorizationRequest`, `PushAuthorizationRequest` and `WritePushedAuthorizationResponse` build a PAR endpoint of your own. Pushed requests are stored with the authorization request methods of `Storage`, so no new methods are needed; a storage that maps fields to columns must persist the new `AuthorizationRequest.Pushed` field and accept IDs of up to 64 bytes that contain `:`.
